...
# Read in data (these are located in: /home/jkimball/haasx092/TajimaD on my MSI account--but are also stored elsewhere, e.g. Google Drive--analysis was carried out on MSI)
...
# this figure doesn't use windowed data so it probably isn't something that you'll want to keep *unless you switch the input to windowed data* but still keeping this so there is a record
# also note that the windowed data didn't really look that great since the nucleotide diversity values are incredibly low
pdf("out.pdf", height = 10, width = 40) # Note: "out.pdf" is simply a placeholder name that I use while testing code to make the figure that I want; th
combined[CLASS == "cultivated", plot(x = POS, y = PI,
			xlab = "Chromosomes",
			...
			type = "o",
			las = 1)]
combined[CLASS == "natural stands", lines(x = POS, y = PI,
			xlab = "Chromosomes",
			ylab = "average nucleotide diversity (pi)",
			ylim = c(0,0.7),
			xaxt = 'n',
