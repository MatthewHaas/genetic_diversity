...
x[scaffold %in% scaffolds_of_interest] -> yy
#y[DP >=6] -> yy
...
# DV columns don't exist in the imputed dataset
#y[GT==0 | GT == 2 | (GT==1 & DV >= 2)] -> z
#yy[GT==0 | GT == 2 | (GT==1 & DV >= 2)] -> z
# Convert from long format to wide format
dcast(yy, scaffold + position + snp_id + ref + alt ~ sample, value.var="GT") -> zz
zz <- as.data.table(zz)
...
save(zz, yy, scaffolds_of_interest, file=args[3])
