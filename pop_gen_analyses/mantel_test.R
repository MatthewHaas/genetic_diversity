...
# Make plots
pdf("220406_mantel_test_.pdf")
plot(mantel, xlab = "Simulated correlation", main = "Mantel test", las = 1)
dev.off()

pdf("220406_dotplot_for_mantel_with_regression.pdf")
...
abline(lm(genet ~ geo), col = "#00a54c", lwd = 2) # running the linear model by itself, e.g. `lm(genet ~ geo)` we find that the intercept is 78.281206 and the 2nd coefficient (geo) is 0.008592
text(x = 500, y = 76, "y = 78.28 + 0.0086x")
text(x = 500, y = 75, expression(italic("R")^"2" ~ " = 0.4149")) # The R^2 value came from running: `summary(lm(genet ~ geo))` -- it gives the R^2, coefficient, residuals, and more
dev.off()
