...
<img src="images/natural_stand_and_breeding_lines_no_GPP_or_GPN_tree_fan.pdf" width="500">

### Figure 4b
Unweighted pair group method with arithmetic averaging (UPGMA) tree for the Natural Stands panel. In this example, 10 samples were chosen from each lake/river to make the tree easier to visualize/interpret. The annotation/labels were manually added in PowerPoint. After the script to create a VCF file with shorter sample names has been run, the scripts [filter_with_vcftools_by_ind_natural_stands_iteration_1.sh](pop_gen_analyses/trees/filter_with_vcftools_by_ind_natural_stands_iteration_1.sh), [filter_with_vcftools_by_ind_natural_stands_iteration_2.sh](pop_gen_analyses/trees/filter_with_vcftools_by_ind_natural_stands_iteration_2.sh), and [filter_with_vcftools_by_ind_natural_stands_iteration_3.sh](pop_gen_analyses/trees/filter_with_vcftools_by_ind_natural_stands_iteration_3.sh) were run to filter the complete VCF file (with all individuals) so that new "slim" VCF files containing only 10 samples from each lake/river are saved (one new VCF file containing different samples from each script). The R script [make_tree_natural_stands_slim.R](pop_gen_analyses/trees/make_tree_natural_stands_slim.R) contains the main code for making the figure (getting data into the right format, bootstraping values, tip label color, etc). It is launched using [run_make_tree_natural_stands_slim.sh](pop_gen_analyses/trees/run_make_tree_natural_stands_slim.sh) which makes the trees for all "slim" natural stand trees.<br>
